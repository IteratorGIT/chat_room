...
void Widget::mouseDoubleClickEvent(QMouseEvent *event)
{
    ...
    if(m_isfull){
        ...
        // 获取centerWidget的中心点
        QPoint widgetCenter = ui->centerWidget->rect().center();
        // 计算移动距离
        QPoint offset = widgetCenter - screenCenter;

        // 移动centerWidget到新位置
        ui->centerWidget->move(offset);
    }
    else {
        m_isfull = true;
...
{
    /*获取群聊列表*/
    writeMsg(nullptr,0,CommandEnum_GetGroupList);
    /*获取群成员信息*/
...
{
    writeMsg(nullptr,0,CommandEnum_GetFriendInfo);
}
...
int Widget::handleGroupChat(void *msg)
{
    ...
    if(groupChatInfo!=nullptr&& userInfo!=nullptr)
    {
...
void Widget::on_pushBtn_send_clicked()
{
    ...
    int isfind = 0;
    int currentIndex = 0;
    QListWidget* chatWidget = nullptr;
    chatWidgetInfo* chatInfo = nullptr;
    ...
    stream << "[" << m_userInfo.m_account << "]" << m_userInfo.m_userName << str.toStdString().c_str();
